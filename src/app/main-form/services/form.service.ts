...
export class FormService {
	...
	getPresentInChild(){
		return this.http.get('http://localhost:4200/app/main-form/services/present.in.child.json').map(response => response.json());
	}

	getPresentInParent(){
		return this.http.get('http://localhost:4200/app/main-form/services/present.in.parent.json').map(response => response.json());
	}
